...
const webhookExp = /^\/webhooks\/[^/ ]+(\/[^ /]+)?/
const guildExp = /^\/guilds\/[^/ ]+/
const channelExp = /^\/channels\/[^/ ]+/
class DiscordAPIError extends Error {
  constructor(rawObj, httpStatus, endpoint, method) {
    super()
    this.name = `DiscordAPIError[${rawObj.code}]`
    function parseErrorRecursive(errors, recursive = "") {
      var message = ""
      var paramName = recursive
      for (var [k, v] of Object.entries(errors)) {
        if (k == "_errors") {
          message += `${paramName}: `
          v.forEach((e, f) => {
            message += `${f== 0 ? "":" ".repeat(paramName.length)}${e.code}: ${e.message}\n`
          })
          return message
        }
        if (!isNaN(parseInt(k))) {
        ...
        message += parseErrorRecursive(errors[k], paramName)
      }
      return message
    }
    var message = rawObj.message
    if (rawObj.hasOwnProperty("errors")) {
      message += `\n${parseErrorRecursive(rawObj.errors, "params")}`
    }
    this.message = message

    this.endpoint = `${method} ${endpoint}`
    this.code = rawObj.code
    this.rawObj = rawObj
...
class APIManager extends EventEmitter {
   ...
   * @returns {null|string} The bucket to request, or null if a new one should be made.
   */
  _resolveBucket(endpoint) {
    var matches = [...(endpoint.match(channelExp) || []), ...(endpoint.match(guildExp) || []), ...(endpoint.match(webhookExp) || [])]
    if (matches.length == 0) return null;
    for (var i of this.endpointBucketMap.keys()) {
      if(i.startsWith(matches[0])) return this.endpointBucketMap.get(i)
    }
    return null
  }
  /**
  ...
  async processGlobalQueue() {
    ...
    while (this.globalRatelimit.remaining > 0 && this.globalRatelimit.queue.length > 0) {
      ...
      if (!this.endpointBucketMap.has(request.endpoint)) {
        var result = this._resolveBucket(request.endpoint)
        if (result == null) {
          this._debug(`[REQUEST] Couldn't associate with a bucket. Creating initial request`)
          await this._request(request)
        } else {
          this._debug(`[REQUEST] Associated with bucket ${result} based on route parameters. Adding that to the bucket handler.`)
          this.bucketRatelimits[result].queue.push(request)
          this.processBucketQueue(result)
        }
      } else {
        this._debug(`[REQUEST] Associated with existing bucket ${result}. Adding that to the bucket handler.`)
  ...
  Method ${request.method}`)
    ...
    if (!res1.ok) {
      ...
      request.reject(new DiscordAPIError(await res1.json(), res1.status, request.endpoint, request.method))
    }
    if (!this.endpointBucketMap.has(request.endpoint)) {
  ...
  Ratelimit will reset in ${resetAfter} seconds`)
    ...
    this.bucketRatelimits[bucket].remaining = remaining
    if (this.bucketRatelimits[bucket].remaining == 0) {
