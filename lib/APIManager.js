...
class APIManager extends EventEmitter {
  ...
  async processGlobalQueue() {
    ...
    if (this.globalRatelimit.total == this.globalRatelimit.remaining) {
      if (this.globalRatelimit.timer) clearTimeout(this.timer)
   ...
   * @private
   */
  onRatelimited({ retry_after, global = false, bucket, request }) {
    this._debug(global ? `[RESPONSE] Uh, we got ratelimited globally. Should retry after in ${retry_after} seconds` : `[RESPONSE] Uh, we got ratelimited on that route
  Bucket: ${bucket}
  ...
  async processBucketQueue(bucket) {
    ...
    if (this.bucketRatelimits[bucket].ratelimited)
      return;
