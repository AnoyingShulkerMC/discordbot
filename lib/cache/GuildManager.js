  ...
  constructor(con, api) {
    ...
    if (con.intents.includes(GatewayConnection.INTENT_FLAGS.GUILDS)) {
      ...
      con.on("GUILD_UPDATE", data => {
        ...
        this.items.set(guild.id, {...this.items.get(data.id), ...guild})
      })
...
function Guild(data, con, api, channels = []) {
  var guild = data
  data.channels = data.channels instanceof GuildChannelManager? data.channels: new GuildChannelManager(con, api, data.id, channels)
  return guild
}